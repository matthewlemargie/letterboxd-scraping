...
def thread(batch):
    first_loop = True
    for batch_idx in tqdm(range(len(batch))):
        page_num = batch[batch_idx]
        driver = webdriver.Firefox()
        driver.get(f"https://www.letterboxd.com/films/popular/page/{page_num}/")
        ...
        for link_idx, link in enumerate(links):
            ...
            film_popularity = (page_num - 1) * 72 + link_idx + 1
        ...
        with finished_lock:
            with open("finished_pages.txt", "w") as f:
                f.write(str(i))
...
batches = [list(range(i, total_pages, 3)) for i in range(1, num_threads + 1)]

threads = []

for batch in batches:
    threads.append(threading.Thread(target=thread, args=(batch,)))
